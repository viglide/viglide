...
/** Unit and property tests for {@link EmaCalculator}. */
class EmaCalculatorTest {

  // ── Anchor test (ADR-0001 §2.2) ─────────────────────────────────────────────────────────────
  // α = 2/(3+1) = 0.5; seed = avg(1,2,3) = 2; then 4·.5+2·.5=3, 5·.5+3·.5=4, 6·.5+4·.5=5.

  @Test
  void anchor_ema3Of1to6_isOffset2Values2345() {
    ...
    IndicatorSeries result = EmaCalculator.calculate(prices, 3);
  ...
  void period1_returnsAllPricesUnchanged() {
    ...
    IndicatorSeries result = EmaCalculator.calculate(prices, 1);
  ...
  void rejectsTooFewPrices() {
    assertThatIllegalArgumentException()
        .isThrownBy(() -> EmaCalculator.calculate(List.of(BigDecimal.ONE), 2));
  }

  @Test
  void rejectsPeriodZero() {
    assertThatIllegalArgumentException()
        .isThrownBy(() -> EmaCalculator.calculate(List.of(BigDecimal.ONE), 0));
  }

  @Test
  void rejectsNullPrices() {
    assertThatNullPointerException().isThrownBy(() -> EmaCalculator.calculate(null, 3));
  }

  // ── Property tests ───────────────────────────────────────────────────────────────────────────

  @Property
  void constantSeries_emaEqualsConstant(
    ...
    IndicatorSeries result = EmaCalculator.calculate(prices, period);
  ...
  @Property
  void emaValuesStayWithinInputRange(
